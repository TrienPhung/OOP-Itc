...
class List_BenhNhan{
	...
	public:
		bool check_Id(int n,string s){ 
			for(int i = 0; i < n;i++){
				if(list[i]->getMa() == s) return false;
			}
			return true;
		}
		void import(int n, int m){
			...
			for(int i = 0; i < n;i++){
				...
				noiTru->import();
				while(check_Id(i,noiTru->getMa()) == false){
					cout << "[!]Ma da ton tai. Vui long nhap lai! " << endl;
					cout << "[-]Nhap ma ho so: ";
					string ma = "";
					fflush(stdin);
					getline(cin,ma);
			...
			for(int i = 0; i < m;i++){
				...
				ngoaiTru->import();
				while(check_Id(list.size(),ngoaiTru->getMa()) == false){
					cout << "[!]Ma da ton tai. Vui long nhap lai! " << endl;
					cout << "[-]Nhap ma ho so: ";
					string ma = "";
					fflush(stdin);
